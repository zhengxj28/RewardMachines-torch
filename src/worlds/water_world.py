...
from src.worlds.game_objects import Actions
...
class WaterWorld:
    ...
    def _get_features_Vis(self):
        ...
        for i in range(2 * num_eyes):
            ...
            features[i * n_features_per_eye:(i + 1) * n_features_per_eye] = get_eye_features(dd, obj, num_classes,
                                                                                             range_max, vel_max)
    ...
    def _get_pos_vel_new_ball(self):
        ...
        if self.use_velocities:
            ...
            vel = 0.0, 0.0
        while True:
            pos = 2 * radius + random.random() * (max_x - 2 * radius), 2 * radius + random.random() * (
                        max_y - 2 * radius)
