...
class PPOAgent(BaseRLAgent):
    ...
    def learn(self):
        ...
        # with torch.no_grad():
        #     v1_nograd = self.critic_net(s1).squeeze(2)
        #     v2_all = self.critic_net(s2).squeeze(2)
        ...
        #         prob_all = self.actor_net(s1[idx])  # shape: (batch, policy_id, action)
        ...
        #
        #         v1 = self.critic_net(s1[idx]).squeeze(2)
...
class ReplayBuffer(object):
    ...
    def sample(self):
        ...
        return s1, a, s2, rs, log_prob, done
